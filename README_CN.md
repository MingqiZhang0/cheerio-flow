...
flowchart TD
    ...
    subgraph GATE["2. 加载门禁"]
        L["加载"]
        L -->|损坏 JSON| BJ["阻止"]
        L -->|重复 ID| DI["阻止"]
        L -->|正常| PG["canPersist = true"]
        BJ --> NG["自动保存禁用<br/>磁盘未修改"]
        DI --> NG
    end

    subgraph SAVE["3. 活跃保存 — v0.1.6 原子化写入"]
        TW[".tmp 写入"]
        FS["flush + sync_all"]
        VT["验证临时文件"]
        RN["重命名"]
        VF["验证目标文件"]
        TW --> FS --> VT --> RN --> VF
        TW -->|重命名前失败| OLD["旧目标文件保留"]
        RN -->|重命名后失败| ERR["报错，无回滚"]
    end

    subgraph DISK["4. 活跃数据 + 布局"]
        V1["v1 扁平<br/>projects/*.json"]
        V2["v2 分组<br/>projects/ungrouped/<br/>projects/groups/"]
        NM["无静默<br/>v1→v2 迁移"]
    end

    subgraph MANIFEST["5. 快照清单 — v0.1.7"]
        direction LR
        SM["原子化清单写入"]
        SH["SHA-256 校验和"]
        WM["警告模式验证"]
...
**第 1–4 层（活跃路径，v0.1.6 增强）：**

- 用户编辑触发自动保存（约 2 秒防抖），首先通过加载门禁。
- 加载失败（损坏 JSON 或重复项目 ID）会禁用持久化门禁——自动保存被阻止，现有文件在磁盘上保持不变。
- 加载成功打开门禁；活跃保存路径通过原子化写入写入项目 JSON、`groups.json` 和 `app-state.json`。
- 原子化写入：写入 `.tmp` → flush → `sync_all` → 验证临时 JSON → 重命名为正式位置 → 验证目标 JSON。
- **重命名前失败**保留旧目标文件。**重命名后验证失败**被检测并报告，但 v0.1.6 未实现重命名后回滚。
- 活跃数据保持其原生布局：v1 扁平或 v2 分组文件夹。v1 自动保存**不会**静默迁移到 v2。

**第 5 层（快照清单，v0.1.7 — 完整性观察）：**

- 每次成功保存后，快照清单被生成并原子化写入。
- 清单记录每个活跃文件的路径、角色、大小和 SHA-256 校验和。
- 加载时，清单以**警告模式**验证：缺失、损坏、校验和不匹配、大小不匹配、额外文件或清单列出但缺失的文件产生警告——绝不会阻止加载。
- 清单写入失败**不会**导致活跃保存失败；以 `manifest/warning` 形式显示。
- 清单是完整性观察层。它**不是**备份、**不是**恢复系统、**不是**加载拦截器。
- 警告消息已脱敏——不会暴露本地绝对路径。

**第 6 层（现有安全路径，v0.1.5 模型 — v0.1.7 未重写）：**

- 备份、恢复、迁移和陈旧隔离保持在 v0.1.5 的暂存/重命名/回滚模型上。

**第 7 层（Storage Console，v0.1.6/v0.1.7 — 只读可观察性）：**

- 从状态栏打开。显示内存存储事件，包括 v0.1.7 的 `manifest/warning` 事件。
- 支持复制日志、清除、关闭、Escape。
- Console 打开时 Ctrl 径向菜单被抑制。
- Console 只读——没有恢复、迁移、删除、修复、重试或重新计算功能。

**第 8 层（延迟——v0.1.7 未包含）：**

- 单写者工作区锁、Recovery Center、自动 `.tmp` 清理和杀进程保存中断验证在 v0.1.7 中**未实现**。
